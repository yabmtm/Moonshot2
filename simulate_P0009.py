...
def prepare_RL_system():
    ...
    platform.setPropertyDefaultValue('CudaDeviceIndex', sys.argv[1])
...
for ligand_ndx in range(len(glob.glob(f'{dataset}_RL/LIG*'))):

    print(f'Processing LIG{ligand_ndx}...')

    try:
        receptor_file = f'{dataset}_RL/fixed_receptor2.pdb'
        RL_output_prefix = f'{dataset}_RL/LIG{ligand_ndx}'
        L_output_prefix = f'{dataset}_L/LIG{ligand_ndx}'
        ligand_file = f'{RL_output_prefix}/LIG_h.sdf'
        ligand_file_pdb = f'{RL_output_prefix}/LIG_h.pdb'
